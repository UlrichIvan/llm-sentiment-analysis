...
To do that, you can use the command below on the root of your projet(`llm-sentiment-analysis` folder) :
...
some terminal cannot show the virtual env. if you use vscode IDE you can see on the status bar below the .venv selected as environment when you open an python file, for example open the `main.py` file on the root of projet, you will see the `.venv` environment selected.
...
After the download completed, you will move the file into the Camembert folder into the `src/models/Camembert` folder.
in this `src/models` folder, you will see others models trained with `scikit-learn` library for the same task`(sentiments analysis)`.

Camembert model is the new trained model with `Hugging Face` library to achieve the same task`(sentiments analysis)`.
...
This projet use the [Streamlit](https://streamlit.io/) library to make all results that you will see as UI Experience. I have also make some custom css to provide the good appear of application like the sidebar and other part of application that you will see.
...
- `Analyses` : this page allow you to make all analyses for your dataset and see the miminal result after it.
- `Results` : this page allow you to see all results associate to all models use in the `Analyses` page to make analyse of
  dataset.
- `Infos` : This page describes the differents steps to achieve the best analyse of your Dataset.
...
Application support four Languages :
...
`Remarks` : you can encounter not results or disabled button analyse when you select and run analyse for the first time with Camembert model, this is because the load of model for the first time take some time, after it the model wil be put into cache by streamlit and the rest of time model should be quickly to load, it's the advantage of cache use into Streamlit.
...
- `Hugging Face model` : the model trained using hugging face Transformers library.

the disference for the two types of models is that :
...
- `Hugging Face model` not necessary need the cleaning text to perform on the of text, because it uses the Transformer architecture, why ? because the goal of the application is to make the `text-classification`,then only the `encoder` can achieve this task, this is because we are selected camembert LLM.

That is the raison you will see the `cleaning step` before the `analyse step` in the `analyses page`, but internaly `Camembert` model not use it to analyse our dataset.
...
In the results pages, into the more results, you will see some words like emojies, url, etc. on the negatives and positives words,but you will never see it for others models, that is because sklearn model use the cleaning dataset and hugging Face not use the cleaned dataset in the case of our application.
...
I added it, to provide the control of user to reset the `Analyse` page when he or she needs or to cancel some disabled utils buttons found on the `Analyse` page.

## Training Camembert model

To see how i will trained Camembert model on CPU, you can see the `llm_sentiments_analysis.ipynb` notebook on the root of projet that describes all steps and analyses necessary to achieve the goal of our projet for Camembert model.

## Summary

Thank you for your attention, for this projet.
If you have any need about IA projets into interships, CDI, CDD applications please contact [me](https://www.linkedin.com/in/ulrich-chokomeny/) by message on linkedIn or recommand me to any company that i will provide my knowledges.<br/>
Best regard.
